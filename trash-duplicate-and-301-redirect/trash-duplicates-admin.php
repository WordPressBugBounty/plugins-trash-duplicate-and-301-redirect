...
if ( ! function_exists( 'tdrd_enqueue' ) ) {
	...
	function tdrd_enqueue() {
		...
		$screen                           = get_current_screen();
		$trash_duplicates_options_version = isset( $trash_duplicates_options['version'] ) ? $trash_duplicates_options['version'] : '1.9';
		if ( 'dashboard' == $screen->id || 'plugins' == $screen->id || ( isset( $_GET['page'] ) && ( 'trash_duplicates' == $_GET['page'] || '301_redirects' == $_GET['page'] || 'trash_log' == $_GET['page'] || 'about_trash_duplicates' == $_GET['page'] || 'trash_duplicates_welcome' == $_GET['page'] || 'trash_duplicates_settings' == $_GET['page'] ) ) ) {
			wp_register_style( 'admin_css', plugins_url( '/css/trash-duplicates.css', __FILE__ ), false, $trash_duplicates_options_version );
			wp_enqueue_style( 'admin_css' );
		}
		wp_enqueue_script( 'admin_js', plugins_url( '/js/trash-duplicates.js', __FILE__ ), array( 'jquery' ), $trash_duplicates_options_version );
		wp_localize_script(
			'admin_js',
			'tdrc_obj',
			array(
		...
		if ( is_rtl() ) {
			wp_enqueue_style( 'admin_rtl_css', plugins_url( '/css/trash-duplicates-rtl.css', __FILE__ ), false, $trash_duplicates_options_version );
		}
	}
}
...
if ( ! function_exists( 'tdrd_admin_func' ) ) {
	...
	function tdrd_admin_func() {
		...
		if ( 0 == $trash_count_result && empty( $trash_duplicates_search_query ) ) :
			?>
			<div class="updated notice below-h1" id="message">
...
<input type="hidden" name="page" value="trash_duplicates" />
					...
					<div class="sol-action-options">
						<select name="duplicates-action-top" class="duplicates-action-top">
							<option selected="selected" class="bold-option" value="none"><b><?php esc_html_e( 'Bulk Actions', 'trash-duplicate-and-301-redirect' ); ?></b></option>
						...
						<select name="trash-post-types">
							...
							foreach ( $trash_duplicates_post_types as $key => $value ) :
								if ( 'wp_block' === $key || 'wp_navigation' === $key || 'shop_order' === $key ) {
									continue;
								}
						...
						<input id="submit-post-type" class="button action" name="filter-post" type="submit" value="<?php esc_attr_e( 'Filter', 'trash-duplicate-and-301-redirect' ); ?>">
					</div>
					<?php if ( $pages_all > 0 ) { ?>
						<div class="tablenav">
...
if ( ! function_exists( 'tdrd_individual' ) ) {
	...
	function tdrd_individual( $trash_ind_id ) {
		if ( wp_trash_post( $trash_ind_id ) === false ) :
			...
			?>
			<div class="error notice below-h1" id="message">
				<p>
					<strong>
...
if ( ! function_exists( 'tdrd_trash_all' ) ) {
	...
	function tdrd_trash_all() {
		...
		if ( isset( $_REQUEST['duplicate-items-apply'] ) && isset( $_REQUEST['delete-all-duplicates'] ) ) {
			...
			$var     = isset( $_REQUEST['keep-all-duplicates'] ) ? absint( $_REQUEST['keep-all-duplicates'] ) : 0;
			$min_max = '';
			if ( 1 == $var ) {
			...
			$post_ids = array();
			$post_ids = isset( $_REQUEST['select_posts_hidden'] ) ? $_REQUEST['select_posts_hidden'] : array();
...
if ( ! function_exists( 'tdrd_log_admin_fun' ) ) {
	...
	function tdrd_log_admin_fun() {
		...
		if ( isset( $_POST['tdrd_log_rest'] ) && isset( $_POST['_wp_tdrd_log_rest_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wp_tdrd_log_rest_nonce'] ) ), '_wp_tdrd_log_rest_action' ) ) {
			$wpdb->query( $wpdb->prepare( 'TRUNCATE TABLE %1s', $table_name ) );       }
		?>
		<div class="tdrd_log_wrap">
			...
			<form class="tdrdlogreset" method="post">
				<?php
				$results = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM %1s', $wpdb->prefix . 'tdrd_log_details' ), ARRAY_A );
				?>
